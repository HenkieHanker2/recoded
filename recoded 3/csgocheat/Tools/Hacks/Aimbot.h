...
private:
	...
	static QAngle CalcAngle(Vector src, Vector dst) {
		...
		g_Math.VectorAngles1(delta, angles);
		return angles;
	}
	bool Fire(CInput::CUserCmd* cmd, Vector vecCurPos, std::vector< Target_t >& possibleaimspots)
	{
		...
		{
			if (Hacks.LocalWeapon->GetActiveBaseCombatWeapon()->GetSpread() > 5 & cmd->buttons & IN_ATTACK)
			{
		...
		if (Options.Ragebot.Aimbot.AutoZeus)
		{
			Vector vecClientTarget;
			bool shoot = true;
			//AUTO ZEUS DES BARRES MAMEN
		/*	if (Options.Ragebot.Aimbot.AutoZeus)
			{
				if (Hacks.LocalWeapon->JJJ()) {
					CCSWeaponInfo* wInfo = Hacks.LocalWeapon->GetCSWpnData(); // Get Weapon Info


					if (wInfo) {
						if (Hacks.LocalPlayer->GetEyePosition().DistTo(vecClientTarget) > wInfo->m_fRange)
							shoot = false;
					}
				}
			}*/
...
public:
	...
	bool Aim(CInput::CUserCmd* cmd)
	{
		...
		std::vector< CBaseCombatWeapon* > Weapon;
