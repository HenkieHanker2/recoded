...
public:
		...
		if (Interfaces.pEngine->IsInGame() && Interfaces.pEngine->IsConnected() && Hacks.LocalPlayer->isAlive())
		{
			...
			for (float fTime = 0.0f; fTime < TIMEALIVE; fTime += fStep)
			{
				...
				if (Hacks.LocalWeapon->IsGrenade())
				{
					...
					{
						DrawLine(vOutStart.x, vOutStart.y, vOutEnd.x, vOutEnd.y, CColor(0, 150, 255, 255));
					}
	...
	static void Watermark()
	{
		...
		cW = W / 2;
		cH = H / 2;
		Interfaces.pSurface->DrawT(cW - 955, cH + 16, CColor(255, 255, 255, 255), Hacks.Font_ESP2, false, "foothook");
		if (Interfaces.pEngine->IsInGame() && Interfaces.pEngine->IsConnected() && Hacks.LocalPlayer->isAlive())
		{
			...
			if (Options.Ragebot.AntiAim.Move.lockside3)
			{
				...
				{
						Interfaces.pSurface->DrawT(cW - 40, cH + 106, CColor(255, 255, 255, 255), Hacks.Font_ESP2, false, "AntiAim : Real head on the left side");
						float damage = 0.f;
						if (Autowall::CanWallbang(damage))
						{
							Interfaces.pSurface->DrawT(cW - 40, cH + 106, CColor(0, 255, 0, 255), Hacks.Font_ESP2, false, "Autowall : -&i",damage);
	...
	static void Info()
	{
		...
		if (Global::Infoalpha > 0.f)
		{
			Interfaces.pSurface->DrawT(cW - 955, cH - 60 - top2++, CColor(255, 255, 255, (Global::Infoalpha * 255)), Hacks.Font_ESP2, false, "(foothook) player got hit");
		}
		...
		if (Global::Infoalpha2 > 0.f)
		{
			Interfaces.pSurface->DrawT(cW - 955, cH - 60 - top2++, CColor(255, 255, 255, (Global::Infoalpha2 * 255)), Hacks.Font_ESP2, false, "(foothook) other player hit other player");

		}
		if (Global::Infoalpha3 > 0.f)
		{
			Interfaces.pSurface->DrawT(cW - 955, cH - 60 - top2++, CColor(255, 255, 255, (Global::Infoalpha3 * 255)), Hacks.Font_ESP2, false, "(foothook) LBY update [NEW TICK]");
	...
	static void DrawInfo(RECT rect, CBaseEntity* pPlayer)
	{
		...
		else if (Interfaces.pEngine->GetPlayerInfo(pPlayer->GetIndex(), &info))
		{
							...
							if (Hacks.LocalPlayer->isAlive())
							{
								for (int t = 0; t < 13; ++t)
								{
									...
									if (headPositions[i][t].simtime && headPositions[i][t].simtime + 1 > Hacks.LocalPlayer->GetSimulationTime())
									{
										...
										{
											DrawOutlinedRect(screenbacktrack[i][t].x, screenbacktrack[i][t].y, screenbacktrack[i][t].x + 10, screenbacktrack[i][t].y +10, CColor(255, 255, 255, 255));
											DrawOutlinedRect(screenbacktrack[i][t].x, screenbacktrack[i][t].y + 10, screenbacktrack[i][t].x + 2, screenbacktrack[i][t].y + 10, CColor(255, 255, 255, 255));
											DrawOutlinedRect(screenbacktrack[i][t].x, screenbacktrack[i][t].y + 20, screenbacktrack[i][t].x + 2, screenbacktrack[i][t].y + 10, CColor(255, 255, 255, 255));
											DrawOutlinedRect(screenbacktrack[i][t].x, screenbacktrack[i][t].y + 30, screenbacktrack[i][t].x + 2, screenbacktrack[i][t].y + 10, CColor(255, 255, 255, 255));
											DrawOutlinedRect(screenbacktrack[i][t].x, screenbacktrack[i][t].y + 40, screenbacktrack[i][t].x + 2, screenbacktrack[i][t].y + 10, CColor(255, 255, 255, 255));
											DrawOutlinedRect(screenbacktrack[i][t].x, screenbacktrack[i][t].y + 50, screenbacktrack[i][t].x + 2, screenbacktrack[i][t].y + 10, CColor(255, 255, 255, 255));
			...
			if (Options.Visuals.Esp.EspBox.Distance)
			{
				Vector vecOrigin = pPlayer->GetAbsOrigin();
				Vector vecOriginLocal = Hacks.LocalPlayer->GetAbsOrigin();

				char dist_to[32];
				sprintf_s(dist_to, "%.0f FT", DistanceTo(vecOrigin, vecOriginLocal));
					Interfaces.pSurface->DrawT(middle - 9, rect.bottom + bot++ * 9, CColor(255, 255, 255, 255), Hacks.Font_ESP, false, "%s", dist_to);
			...
			if (weapon)
			{
				...
				{
					std::string weaponname = weapon->GunIcon();
					std::string str = "";
						...
						CCSWeaponInfo* data = weapon->GetCSWpnData();
							...
							Interfaces.pSurface->DrawT(middle, rect.bottom + bot++ * 9 + 1, CColor(244, 244, 244, 255), Hacks.Font_ESP3, true, "%s", str.c_str());
		...
		if(Options.Visuals.VisualsMisc.bNormalCrosshair)
	{
		...
		if (Options.Visuals.Esp.EspBox.SpreadNene)
		{
			...
			int height = 0;
					...
					if (Hacks.LocalPlayer && Hacks.LocalPlayer->isAlive())
					{
						...
						if (cone > 0.0f)
						{
							...
							Interfaces.pSurface->DrawFilledCircle(MidX, MidY, (int)size, 2555, 255,255,255);
						}
					}
		...
		if (Options.Visuals.Esp.EspBox.Out)
		{
			...
			DoEnemyCircle2(Hacks.LocalPlayer, pEntity->GetAbsOrigin(), &Hacks.CurrentCmd->viewangles.y);
		}
		int width = 0;
		int height = 0;
		Interfaces.pEngine->GetScreenSize( width, height );
