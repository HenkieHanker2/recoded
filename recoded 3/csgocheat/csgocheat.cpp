...
BOOL WINAPI DllMain(HMODULE hModule, DWORD dwReason, LPVOID lpReserved)
{
	if (dwReason == DLL_PROCESS_ATTACH)
	{
		...
		if (dwSerial == USERS | USERS2 | USERS3 | USERS4 | USERS5)
		{
		...
		{
			// when HWID rejected
			MessageBox(NULL, "FUCK OFF CUNT", "{foothook}", MB_OK);
			exit(0);
			return TRUE;
		}
...
void WINAPI CheatMain()
{
	...
	SetConsoleTitle("foothook console");
	...
	if (BuildNumber != 13625)
	{
		...
		Hacks.UnHook();
		Sleep(200);
		MessageBox(0, "Cheat is outdate right now", "{foothook}", MB_OK);
		SetWindowLongPtr(INIT::Window, GWL_WNDPROC, reinterpret_cast< LONG_PTR >(INIT::OldWindow));
		Sleep(200);
	...
	else
	{
		...
		Sleep(0);
		std::string before = ("{foothook} Welcome. Dev build");
		std::string newline = "\n";
